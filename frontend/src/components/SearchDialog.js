...
const SearchDialog = (props) => {
    ...
    const handleSearch= () => {
        ...
        if (!authToken) {
            ...
            if (isPeriodic) {
                ...
                searchObj.period.from = ((startDatePeriod.getDay() + 6) % 7) + 1;
    ...
    const canSearch = () => {
        if (!authToken) {
            if(place) return {disabled: false}
            else return {disabled: true}
        } else {
            if(!isPeriodic) {
                if(place && startDate && endDate && startTime && endTime) return {disabled: false}
                else return {disabled: true}
            } else {
                if(place && startDatePeriod && weekPeriod && daysPeriod) return {disabled: false}
                else return {disabled: true}
            }
        }
    }
    ...
    return (
        ...
        >
            {authToken ? (
                ...
                <>
                    <label htmlFor="place-search-bar" className="font-medium">
                        Seleziona un punto di ritiro
                    </label>
                    <div className="border mt-2">
                        <Select
                    ...
                    <div class="alert alert-info">
                        <div class="flex-1">
                            ...
                            <label>
                                Solo gli utenti registrati possono ricercare le
                                auto per data
                            </label>
                        </div>
                    </div>
